...
:::tip[不會寫程式也能參與嗎？]
完全沒問題！修錯字、補資訊等簡單的內容修改，全程都能在網頁上點擊操作，也不用背程式碼指令。下面的「讓 AI Agent 幫你改」與「方式二」都是為你寫的，跟著步驟走就能完成第一次貢獻。
:::
...
:::steps
1. **把專案抓下來**：`git clone https://github.com/NTUST-OpenSource/freshman.git`，在專案資料夾裡開你的 AI Agent。
...
4. **建立 PR**：目標分支自動設成 `dev`，關聯剛剛那則 Issue，並指定審查者與標籤。
5. **盯審查結果**：PR 開好之後會自動觸發程式碼審查。
:::

:::warning[看到 5/5 才算完成]
審查會給一個信心分數，要到 **5/5** 而且沒有未處理的留言，才算真的提交完成。分數不到就代表還有東西要修。AI Agent 會在背景持續盯著審查狀態，抓到問題會主動告訴你哪裡要改，改完推上去會重新審一次。開完 PR 就把視窗關掉是不夠的。
:::
...
* **私下聯絡**：如果有不便公開的事項，可以到[銘謝頁](/thanks/)找到維護者的 GitHub 帳號，從個人頁面與對方聯絡。
