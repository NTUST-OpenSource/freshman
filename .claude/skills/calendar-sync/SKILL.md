...
- 網站用資料：`src/data/calendar-{學年}.json`（從 parsed 複製，目前僅 115）
...
# 115 需同步網站副本
cp docs/calendar/parsed/115.json src/data/calendar-115.json
```
...
5. sync 統計合理性：titleKept 數量異常暴增代表舊檔被誤判為人工編輯；manualAdded 出現非預期 id 代表 id 偏移
